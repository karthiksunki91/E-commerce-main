...
export default function SignupPage() {
  ...
  return (
    <div className="flex-grow flex min-h-[85vh] bg-white dark:bg-zinc-950 animate-fadeIn flex-row-reverse">
      {/* Right side Image (Reversed for variation) */}
      <div className="hidden lg:block lg:w-1/2 relative bg-slate-900">
        <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-black/20 to-transparent z-10"></div>
        <img 
        ...
        <div className="absolute bottom-12 right-12 z-20 text-white text-right">
          <h2 className="text-4xl font-extrabold mb-4">Join the Club</h2>
          <p className="text-lg opacity-80 max-w-md ml-auto">Create an account today to discover exclusive collections and member-only pricing.</p>
        </div>
      </div>
        ...
        <div className="max-w-md w-full">
          ...
          <form className="space-y-6" onSubmit={handleSignup}>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full flex justify-center py-4 px-4 border border-transparent text-lg font-bold rounded-xl text-white bg-slate-900 dark:bg-white dark:text-zinc-900 hover:bg-primary dark:hover:bg-primary dark:hover:text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary disabled:opacity-50 transition-all shadow-xl shadow-slate-900/20 mt-8"
            >
              {loading ? (
